  ...
  Future<void> _loadTripData() async {
    ...
    for (String question in allQuestions) {
      controllers[question] =
          TextEditingController(text: details?[question] ?? '');
    }

    // Handle separate date fields
    controllers['startDate'] =
        TextEditingController(text: details?['startDate'] ?? '');
    controllers['endDate'] =
        TextEditingController(text: details?['endDate'] ?? '');
              ...
              children: questions.map((question) {
                if (question == "What are your travel dates (start and end)?") {
                  ...
                  return _buildDropdown(
                      question, controllers[question]!, predefinedOptions[question]!);
                } else {
                  return _buildTextField(question, controllers[question]!);
                }
              }).toList(),
